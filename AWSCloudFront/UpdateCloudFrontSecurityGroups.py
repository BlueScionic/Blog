...
GLOBAL_SG_TAGS1 = { 'Name': 'cloudfront_g1', 'AutoUpdate': 'true' }
GLOBAL_SG_TAGS2 = { 'Name': 'cloudfront_g2', 'AutoUpdate': 'true' }
REGION_SG_TAGS1 = { 'Name': 'cloudfront_r1', 'AutoUpdate': 'true' }
REGION_SG_TAGS2 = { 'Name': 'cloudfront_r2', 'AutoUpdate': 'true' }
...
def lambda_handler(event, context):
    ...
    global_cf_ranges_full = get_ranges_for_service(ip_ranges, SERVICE, "GLOBAL")
    region_cf_ranges_full = get_ranges_for_service(ip_ranges, SERVICE, "REGION")
    
    # Split ranges into two groups
    global_cf_ranges1, global_cf_ranges2 = split_ip(global_cf_ranges_full)
    region_cf_ranges1, region_cf_ranges2 = split_ip(region_cf_ranges_full)

    # Update the security groups
    result = update_security_groups(global_cf_ranges1, "GLOBAL_A")
    result = result + update_security_groups(global_cf_ranges2, "GLOBAL_B")
    result = result + update_security_groups(region_cf_ranges1, "REGION_A")
    result = result + update_security_groups(region_cf_ranges2, "REGION_B")
...
def update_security_groups(new_ranges, rangeType):
    ...
    for curGroup in allSGs:
        tagToFind = {}
        if rangeType == "GLOBAL_A":
            tagToFind = GLOBAL_SG_TAGS1
        elif rangeType == "GLOBAL_B":
            tagToFind = GLOBAL_SG_TAGS2
        elif rangeType == "REGION_A":
            tagToFind = REGION_SG_TAGS1
        else:
            tagToFind = REGION_SG_TAGS2
